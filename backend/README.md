...
**Leafy Wallet Backend is the enrichment API for our offline-first wallet demo**, showcasing MongoDB features tailored for [Financial Services](https://www.mongodb.com/solutions/industries/financial-services). It is a [FastAPI](https://fastapi.tiangolo.com/) service, managed with [uv](https://docs.astral.sh/uv/), that owns the read side of the wallet data living in [MongoDB Atlas](https://www.mongodb.com/atlas): contacts, and the transaction history collection with its search indexes. Balances and real transfers live in the Payment Platform (PSP), not here; this service holds no money-moving credentials.

## Components and Features:

1. **Wallet collections API**
   - Read-only routers for contacts and transactions. Writes reach Atlas through ObjectBox Sync, not through here.
...
4. **Fraud signals**
   - `$setWindowFields` counts sends inside a rolling window to flag bursts, the account-takeover shape the device cannot compute for itself.
...
Each router prefix maps to its own Atlas collection: `/api/v1/wallet-contacts` and `/wallet-transactions`. Both are GET-only. `/wallet-transactions/search` runs hybrid search over `walletTransactionsHistory`, `/wallet-transactions/summary` runs the spending aggregation, and requests and chats are read from the device rather than over HTTP.
...
- **Database Triggers** copy each settled transfer into `walletTransactionsHistory`, so the searchable long tail accumulates in Atlas with no application code writing it.
- **Window functions** (`$setWindowFields`) count sends inside a rolling window to flag payment bursts, an analysis with no on-device equivalent.
...
- Hybrid search needs both the vector and the Atlas Search index to exist; run `uv run python scripts/create_vector_index.py` if searches return nothing. It's idempotent, so re-running is safe.
