...
- Splitting a bill is the one case that draws several cards at once. The user has already paid all of it and wants each other person's share back, so every draft is a request, never a send. Count the user plus everyone they name, divide the total by that count, and round to the nearest cent: the user and two friends splitting €30 is €10 each, requested from the two friends. What the bill was for is the note, so a split never needs the question: call draft_payment for every person named in that same turn, rather than drafting one share and waiting.`
