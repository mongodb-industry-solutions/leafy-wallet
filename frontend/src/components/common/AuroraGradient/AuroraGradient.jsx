...
/**
 * Soft rainbow glow anchored to the bottom that rises up on mount via a
 * scaleY(0) → 1 transform, like an aurora unfurling from the floor.
 * @param {object} props
 * @param {number} [props.bars] - Number of blurred gradient columns.
 * @param {number} [props.blur] - Gaussian blur radius per column.
 * @param {number} [props.peak] - Max column height as a fraction of the viewBox.
 * @param {number} [props.valley] - Min column height (at the bell curve's edges).
 * @param {{offset: number, color: string}[]} [props.stops] - Gradient stops, bottom to top.
 * @param {number} [props.riseMs] - Duration of the rise-up transition.
 */
export function AuroraGradient({
  ...
  useEffect(() => {
    const id = requestAnimationFrame(() => requestAnimationFrame(() => setIsShown(true)))
  ...
  return (
    <div
      ...
      style={{
        ...
        transform: isShown ? 'scaleY(1)' : 'scaleY(0)',
