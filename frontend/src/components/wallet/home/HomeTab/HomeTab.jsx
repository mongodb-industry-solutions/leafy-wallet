...
// Other balances kept for when MULTI_CURRENCY_ENABLED is turned back on.
const ALL_CARDS = [
  { currency: '€', amount: '12,458.32', sub: 'Sending €45.00', isPending: true, last4: '7984', tone: 'green' },
...
const CARDS = MULTI_CURRENCY_ENABLED ? ALL_CARDS : ALL_CARDS.slice(0, 1)
...
/**
 * The "Home" tab: balance card(s), Send/Request/Chat shortcuts, and a
  ...
  return (
    <div className="flex flex-col gap-5 px-4 pt-2 pb-6">
      <div className="no-scrollbar -mx-4 flex snap-x snap-mandatory gap-3 overflow-x-auto scroll-px-4 px-4">
        {CARDS.map((c, i) => {
          const subGlyph = c.isPending ? 'Pending' : c.sub.startsWith('-') ? 'Minus' : 'Plus'
          const subText = c.isPending ? c.sub : c.sub.replace(/^[+-]\s?/, '')
          return (
            <div
              key={i}
              className={cn(
                'flex flex-none snap-start flex-col rounded-3xl p-5',
                CARDS.length > 1 ? 'w-[86%]' : 'w-full',
                TONES[c.tone],
              )}
            >
